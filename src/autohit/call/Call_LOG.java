...
/**
 * LOG call.  Makes a log entry.
 * <pre>
 * REQURIES: logger, core
 * IGNORES: uni
 * PARAMETERS (INPUT):
 *	entry= text entry
 *	numeric= value (optional).  Value between 
 * </pre>
 ...
 */
public class Call_LOG extends Call {
	...
	public String call() throws CallException {
		...
		try {
			int numeric = CallException.CODE_INFORMATIONAL_OK;

			String numString = this.optionalString("numeric");
			if (numString != null) {
				try {
					numeric = Integer.parseInt(numString);
				} catch (Exception ee) {
					throw new CallException(
						this.format(
							"Numeric format is bad.  String=" + numeric),
						CallException.CODE_CALL_PROGRAM_ERROR);
				}
			}
			// DO NOT USE Call SERVICE for this
			log.info(((String) vmc.fetch("entry")), numeric);
			...
			throw new CallException(
				this.format(
					"Exception while logging.  error=" + e.getMessage()),
