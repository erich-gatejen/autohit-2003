...
/**
 * CALL.  REMOVE_TABLE  Remove a table in persist.
 * <pre>
 ...
 */
public class Call_REMOVE_TABLE extends Call {
	...
	public String call() throws CallException {
		...
		try {
			...
			if (vmc.has(name)) {
				vmc.free(name);
				this.debug("Removed a table named=" + name);
			} else {
				this.debug(
					"Table "
						+ name
						+ " not in persist.  Obviously, I cannot remove it.");
			}
